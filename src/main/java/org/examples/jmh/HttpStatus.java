...
public enum HttpStatus {
	...
	public static HttpStatus toCode(String rawCode) {
		if (StringUtils.isBlank(rawCode)) {
			return HttpStatus.I_AM_A_TEAPOT;
		}

		return Arrays.stream(HttpStatus.values())
				.filter(code -> code.name().equalsIgnoreCase(rawCode))
				.findFirst()
				.orElse(HttpStatus.I_AM_A_TEAPOT);
	}

	public static HttpStatus toCodeAlternative(String rawCode) {
		if (StringUtils.isBlank(rawCode)) {
			return HttpStatus.I_AM_A_TEAPOT;
		}

		String upperCaseRawCode = rawCode.toUpperCase();
		try {
			return HttpStatus.valueOf(upperCaseRawCode);
		} catch (Exception ex) {
			return HttpStatus.I_AM_A_TEAPOT;
		}
